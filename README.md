# An example of Android multi-module project architecture using MVVM design pattern + Clean Architecture + Jetpack Compose (UI) for client-server apps
...
There are some unit tests but it's still in progress - more will be added later.

To make app fetch data from network data store you need to add Unsplash API keys (https://unsplash.com/developers) to *local.properties*:
```
...
Below some explanations of modules and code examples (it can be similar to the project code itself, just to show the idea, but also something can be outdated in the examples here)
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // then we use Jetpack Compose to implement all the needed screens
        setContent { 
            ArchApp()
        }
    }
...
class AuthNetworkDataSourceImpl(
    ...
    override suspend fun login(email: String, password: String): LoginResponse = try {
        ...
        response.data.asExternalModel() // use separate network models and then map them to POJO business external models
    } catch (e: Throwable) {
...
```
Adding a bearer autorization token using OkHttp interceptor:
```
